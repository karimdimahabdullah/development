...
export function translate2d(dx, dy) {
  ...
  return function translatedCoordinatePair(x, y){
      return [x + dx, y + dy]
  }
...
export function scale2d(sx, sy) {
  ...
  return  function scaleCoordinatePair (x, y) {
    return [x * sx, y * sy]
  }
...
export function composeTransform(f, g) {
    ...
    return function compositionFunction (x, y) {
      return g(...f(x, y))
    }
...
export function memoizeTransform(f) {
  ...
  return function memoizeTransformFunc(x, y) {
    if(lastArgs !== null) {
      let isSubset = (array1, array2) => array1.every(
        (element, index) => element === array2[index]
      )
      let hasValue = isSubset(lastArgs, [x,y])
      if  (hasValue){
        return lastResults
      }else {
        lastArgs = [x, y]
